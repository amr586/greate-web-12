...
async function notifyPurchaseRequest(propertyId: number, buyerId: number, amount: number, paymentMethod: string) {
  try {
    const propRes = await query('SELECT title, title_ar, district FROM properties WHERE id=$1', [propertyId]);
    ...
    const notifMessage = `طلب شراء جديد - العقار: ${title} | المشتري: ${buyer.name} (${buyer.phone}) | المبلغ: ${Number(amount).toLocaleString()} جنيه | طريقة الدفع: ${paymentMethod === 'instapay' ? 'InstaPay' : 'فودافون كاش'}`;
    const adminsRes = await query(
      `SELECT id FROM users WHERE role='superadmin' OR (role='admin' AND (sub_role='property_manager' OR sub_role IS NULL))`
    );
    for (const admin of adminsRes.rows) {
      await query(
        ...
        [
          ...
          'طلب شراء جديد',
          notifMessage,
          JSON.stringify({ id: propertyId, title, district: prop.district, price: amount }),
          JSON.stringify({ id: buyerId, name: buyer.name, email: buyer.email, phone: buyer.phone }),
        ]
      );
    }
  } catch (error) {
...
router.post('/', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    ...
    await notifyPurchaseRequest(property_id, req.user!.id, amount, payment_method);

    res.status(201).json({
      payment: result.rows[0],
      walletInfo: {
        instapay: '+201100111618',
        vodafone: '+201100111618',
        message: `يرجى تحويل مبلغ ${Number(amount).toLocaleString()} جنيه على ${payment_method === 'instapay' ? 'InstaPay' : 'فودافون كاش'}: +201100111618`
      }
    });
  } catch (err) {
    console.error(err);
